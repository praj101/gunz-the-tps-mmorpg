...
bool RMesh::ReadXmlElement(CCXmlElement* PNode,char* Path)
{
	char NodeName[256];
	char IDName[256];
	char FileName[256];
...
/*
	if(t_hd.ver != EXPORTER_VER) {
		cclog("%s elu file 버젼이 틀림.\n",fname);
		return false;
	}
*/
	if(t_hd.sig != EXPORTER_SIG) {
		cclog("%s elu file 파일 식별 실패.\n",fname);
		return false;
	}
...
//		node->m_power = 80.f;
		...
		if(t_hd.ver > EXPORTER_MESH_VER7 )//ver8 부터
		{
			...
			node->m_iAlphaTestValue = alpha_test;
		}
...
//			result_mat2 = scalepivotinv * scalemat * scalepivot;
		...
		{
			cclog( "m_data number is not quite right..! (%d)\n", (int)m_data.capacity());
		}
	}
