...
bool CCFont::Create(const char* szName)
{
	_ASSERT(strlen(szName)<CCFONT_NAME_LENGTH);
	strcpy(m_szName, szName);

	return true;
}

void CCFont::Destroy(void)
{
}

int CCFont::GetWidthWithoutAmpersand(const char* szText, int nSize)
{
...
/*
int CCFont::GetPossibleCharacterCount(int nRectWidth, const char* szText)
{
	...
	for(int i=0;i<nLength;i++)
	{
		...
		char *puretext=CCDrawContext::GetPureText(text);
