...
class CCMatchDayOfMonth
{
public :
	CCMatchDayOfMonth();
	~CCMatchDayOfMonth();

	static CCMatchDayOfMonth& GetInst()
	{
		static CCMatchDayOfMonth inst;
		return inst;
	}
...
class CCMatchScheduleData
{
public:
	// 스케쥴의 종류는 1:repeat, 2:Count, 3:Once로 되어있음.
	enum CCMatchScheduleState
	{
		REPEAT = 0,
		COUNT,
		ONCE,
...
class CCMatchScheduleReleaser
{
public :
	void operator() ( CCMatchScheduleData*& rfSchedulerData )
	{
...
{
public :
	virtual ~CCMatchScheduleImpl() {}

	// 시간 오차 보정.
	virtual void CorrectTime( CCMatchScheduleData* pScheduleData ) = 0;
	// 시간을 다시 설정함.
	virtual void Reset( CCMatchScheduleData* pScheduleData ) = 0;
};
// 제거되지 않고 계속 반복되는 스케쥴.
class CCMatchRepeatScheduleImpl : public CCMatchScheduleImpl
{
public :
	// 시간 오차 보정.
	void CorrectTime( CCMatchScheduleData* pScheduleData );
	// 시간을 다시 설정함.
	void Reset( CCMatchScheduleData* pScheduleData );
};
// 일정 횟수를 실행하고 제거되는 스케쥴.
class CCMatchCountScheduleImpl : public CCMatchScheduleImpl
{
public :
	// 시간 오차 보정.
	void CorrectTime( CCMatchScheduleData* pScheduleData );
...
// 한번만 실행하고 제거되는 스케쥴.
class CCMatchOnceScheduleImpl : public CCMatchScheduleImpl
{
public :
	// 시간 오차 보정.
	void CorrectTime( CCMatchScheduleData* pScheduleData );
...
typedef vector< CCMatchScheduleData* > ScheduleVec;
typedef ScheduleVec::iterator	ScheduleVecIter;

typedef list< CCMatchScheduleData* >	ScheduleLst;
...
{
public :
	CCMatchScheduleMgr( MServer* pServer );
	~CCMatchScheduleMgr();

	// 동적인 스케쥴 등록. 조건에 의해서 지워질수 있음.
	bool AddDynamicSchedule( CCMatchScheduleData* pNewSchedule );
	// 시스템이 시작하면서 종료될때까지 유지되어야 하는 스케쥴.
	bool AddStaticSchedule( CCMatchScheduleData* pNewSchedule );

	// Schedule data maker.
	CCMatchScheduleData* MakeRepeatScheduleData( const unsigned char	cNextYear,	// 다음에 실행될 해의 상대값.	ex) 1년 후.
...
private :
	...
	// 정상적적인 데이터인지 검사함.
	bool CheckData( CCMatchScheduleData* pScheduleData ) const;
	// 업데이트 되어야 하는 시간인지 검사함.
	bool CompareTime( CCMatchScheduleData* pScheduleData );
	// 업데이트가 가능한지 검사.
	bool IsUpdate();
	...
	// 수행자를 가지고 있는 저장 클래스.
	class CCMatchScheduleImplPrototype
	{
	private :
		friend CCMatchScheduleMgr;

		typedef vector< CCMatchScheduleImpl* >	ScheduleImplVec;

		CCMatchScheduleImplPrototype() {}
		~CCMatchScheduleImplPrototype() {}

		bool Init();
		// 해당 타입ID에 맞는 수행자의 포인터를 넘겨줌. 절대 delete하면 않됨.
		CCMatchScheduleImpl* GetImpl( const unsigned int nTypeID );
		void Release();
	...
	class CCMatchScheduleImplRelease
	{
	public :
		void operator() ( CCMatchScheduleImpl*& rfImpl )
		{
			if( 0 != rfImpl ){
				delete rfImpl;
				rfImpl = 0;
...
private :
	...
	CCMatchScheduleImplPrototype	m_ScheduleImplPrototype;

	vector< CCMatchScheduleData* >	m_vecStaticSchedule;	// 시스템 가동시 등록되는 스케쥴. 시스템수명과 같음.
	list< CCMatchScheduleData* >		m_lstDynamicSchedule;	// 공지사항.
};
