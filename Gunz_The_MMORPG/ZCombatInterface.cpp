...
//	DrawResultBoard(pDC);
	...
	if ( !m_bShowResult && IsFinish())
	{
		...
		if ( timeGetTime() >= m_nReservedOutTime)
		{
			...
			if ( pWidget)
				pWidget->Show( false);
			CCLabel* pLabel = (CCLabel*)ZApplication::GetGameInterface()->GetIDLResource()->FindWidget( "CombatChatInput");
			if ( pLabel)
				pLabel->SetText("");
...
void ZCombatInterface::OnDrawCustom(CCDrawContext* pDC)
{
	...
	if ( m_bShowResult)
	{
		...
		// 남은 시간을 표시한다
		CCLabel* pLabel = (CCLabel*)ZGetGameInterface()->GetIDLResource()->FindWidget( "GameResult_RemaindTime");
		if ( pLabel)
		{
...
void AddCombatResultInfo( const char* szName, int nScore, int nKill, int nDeath, int bMyChar, bool bGameRoomUser)
{
	...
	for ( int i = 0;  i < 16;  i++)
	{
		...
		CCLabel* pLabel = (CCLabel*)ZApplication::GetGameInterface()->GetIDLResource()->FindWidget( szWidget);
		if ( pLabel)
		{
...
void ZCombatInterface::GetResultInfo( void)
{
	...
	for ( int i = 0;  i < 16;  i++)
	{
		...
		if ( pLabel)
		{
			...
			pLabel->SetText( "");
			pLabel->SetAlignment( CCD_LEFT | CCD_TOP);
		}
	...
	else if ( ZGetGameClient()->IsLadderGame())		// 클랜전이면...
	{
		...
		for ( int i = 0;  i < 2;  i++) 
		{
			...
			CCLabel* pLabel;
			if ( nWinnerTeam == nTeam)
				pLabel = (CCLabel*)ZApplication::GetGameInterface()->GetIDLResource()->FindWidget( "ClanResult_PlayerNameListLabel1");
			else
				pLabel = (CCLabel*)ZApplication::GetGameInterface()->GetIDLResource()->FindWidget( "ClanResult_PlayerNameListLabel2");
			if ( pLabel)
			{
