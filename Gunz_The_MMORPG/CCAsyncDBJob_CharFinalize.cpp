...
void CCAsyncDBJob_CharFinalize::Run(void* pContext)
{
	CCMatchDBMgr* pDBMgr = (CCMatchDBMgr*)pContext;

	CCASYNC_RESULT nResult = CCASYNC_RESULT_SUCCEED;

	if (!pDBMgr->UpdateCharPlayTime(m_nCID, m_nPlayTime)) {
		nResult = CCASYNC_RESULT_FAILED;
	}
...
bool CCAsyncDBJob_CharFinalize::Input(DWORD nAID,
									 int nCID, 
