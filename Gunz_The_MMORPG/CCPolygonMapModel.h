...
friend class MPolygonMapModel;
	...
	CCPtrList<MPMPoint>	m_VisibleList;		// 보이는 점 리스트
	...
	CCPtrList<MPMPoint>	m_VisibledList;		// 이점이 보이지는 다른 점 리스트
	void*				m_pTemp;			// 임시 포인터 ( 보장받지 않는다. )

	//void*				m_pRoomIndex;
	list<int>			m_RoomIndexs;
	CCPtrList<MPMPortal>	m_InPortals;
...
class MPMPolygon{
public:
	CCPtrList<MPMPoint>		m_PointList;		// 각 모서리
...
protected:
	// Merge Support
	CCPtrList<MPMPoint>		m_TempMergedPointAddList;		// 머지를 위한 임시 폴리곤 리스트
	CCPtrList<MPMPoint>		m_TempMergedPointDupList;		// 머지를 위한 임시 폴리곤 리스트, 겹치는 포인트
...
public:
	...
	void InsertBefore(int nIndex, CCPtrList<MPMPoint>* pList);
	void AddAfter(int nIndex, CCPtrList<MPMPoint>* pList);
...
public:
	...
	CCLinkedList<MPMPoint>		m_PointList;
	CCLinkedList<MPMPolygon>	m_PolygonList;
...
protected:
	...
	bool FindNonIntersectPosition(CCPtrList<MPMPolygon>* pPolygons, float* rx, float* ry, float x, float y);
...
public:
	...
	MPMPolygon* GetNearestInnerPolygon(CCLinkedList<MPMPolygon>* pPolygons, float x, float y);	// Inner Polygon중 가장 가까이에 있는 폴리곤( 방향에 상관 없음 ) (1, 0, 0) 방향이다.

	bool IsEnable(CCLinkedList<MPMPolygon>* pPolygons, float x, float y);	// Enable Position인가?
