...
void HLineBitmap(CCDrawContext* pDC, int x, int y, int w, CCBitmap* pBm, bool bStretch, float fScale)
{
	if(pBm==NULL) return;
...
void VLineBitmap(CCDrawContext* pDC, int x, int y, int h, CCBitmap* pBm, bool bStretch, float fSclae = 1.f)
{
	if(pBm==NULL) return;
	int bw = pBm->GetWidth();
...
void DrawBitmapFrameV3(CCDrawContext* pDC, sRect& r, CCBitmap* Bms[3])
{
	if(IsNull(Bms, 3)==true) return;
