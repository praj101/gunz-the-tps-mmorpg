...
void CCPicture::OnDraw(CCDrawContext* pDC){
 	if(m_pBitmap==NULL) 
		return;
	pDC->SetBitmap(m_pBitmap);

	sColor tempColor = pDC->GetBitmapColor();
...
void CCPicture::SetAnimation( int animType, float fAnimTime ){
	m_bAnim = true;
...
void CCPicture::OnAnimDraw( CCDrawContext* pDC, int x, int y, int w, int h, int bx, int by, int bw, int bh ){
...
CCPicture::CCPicture(const char* szName, CCWidget* pParent, CCListener* pListener)
...
void CCPicture::SetBitmap(CCBitmap* pBitmap){
	m_pBitmap = pBitmap;
...
void CCPicture::SetBitmapColor( sColor color ){
