...
bool RNavigationMesh::Open(const char* szFileName, CCZFileSystem* pZFileSystem)
{
	CCZFile file;
...
{
	if (m_iVertCount <= 0) return false;
	...
	// vertex -------------
	fwrite(&m_iVertCount, sizeof(int), 1, file);
	...
	// face ---------------
	fwrite(&m_iFaceCount, sizeof(int), 1, file);
	for (int i = 0; i < m_iFaceCount; i++)
	{
