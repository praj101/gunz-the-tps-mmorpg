...
#ifndef _PUBLISH
	cclog("D3D_SDK_VERSION = %d \n", D3D_SDK_VERSION);
#endif

	cclog("Display Device = %s ( vendor=%x device=%x subsys=%x revision=%x )\n", 
		deviceID.Description,deviceID.VendorId,deviceID.
		DeviceId,deviceID.SubSysId,deviceID.Revision);

	cclog("Display Driver Version = %d.%d.%04d.%04d\n",
