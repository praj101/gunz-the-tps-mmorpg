...
{
	m_iWidth = TEXTURE_SIZE;
	m_iHeight = TEXTURE_SIZE;
	HRESULT hr = RGetDevice()->CreateTexture(m_iWidth, m_iHeight, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, &m_pTexture,NULL);
...
{
	m_iX = m_iWidth/m_iCellSize;
	m_iY = m_iHeight/m_iCellSize;

	m_iCell = m_iX * m_iY;
	m_CellInfo = new RFONTTEXTURECELLINFO[m_iCell];
	for(int i=0;i<m_iCell;i++) {
		m_CellInfo[i].nID = 0;
	...
	bmi.bmiHeader.biWidth       =  (int)m_iCellSize;
	bmi.bmiHeader.biHeight      = -(int)m_iCellSize;
...
bool RFontTexture::UploadTexture(RCHARINFO *pCharInfo,DWORD* pBitmapBits,int w,int h)
{
	...
	if(hr == D3D_OK)
	{
		...
		// 실제 픽셀의 위치
		int _x = x*m_iCellSize;
		int _y = y*m_iCellSize;

		BlitRect((BYTE*)d3dlr.pBits, _x, _y, _x+w, _y+h, m_iCellSize, m_iCellSize, pBitmapBits, d3dlr.Pitch);
...
bool RFontTexture::MakeFontBitmap(HFONT hFont, RCHARINFO *pInfo, const TCHAR* szText, int nOutlineStyle, DWORD nColorArg1, DWORD nColorArg2)
{
	...
	int nWidth = min(size.cx,m_iCellSize);
	...
	else if (nOutlineStyle == 1)	// 아웃라인이 그려진다
	{
		...
		int nHeight;
		nHeight = min( (int)tm.tmHeight, (int)m_iCellSize-2);
	...
	bool bRet = UploadTexture(pInfo,m_pBitmapBits,nWidth,m_iCellSize);
//	_ASSERT(bRet);
...
//	m_iSamplingMultiplier = nSamplingMultiplier;	// 한방향 샘플링 횟수, 2이면 2*2크기를 샘플링한다.
...
//		fScale = RGetScreenWidth() / 800.f;
	...
	while(1) {
		...
		if(i==m_CharInfoMap.end()) {
			pInfo = new RCHARINFO;
			bool bRet = m_pFontTexture->MakeFontBitmap(m_hFont,pInfo,szChar,m_iOutlineStyle,m_ColorArg1,m_ColorArg2);
			if(bRet)
		...
		}else
		{
			...
			if(m_pFontTexture->IsNeedUpdate(pInfo->nFontTextureIndex,pInfo->nFontTextureID)) {
				m_pFontTexture->MakeFontBitmap(m_hFont,pInfo,szChar,m_iOutlineStyle,m_ColorArg1,m_ColorArg2);
			}
		}
...
/*
			...
			vertices[1].p = D3DXVECTOR4(x,y+m_iCellSize,0,1);
			vertices[2].p = D3DXVECTOR4(x+nWidth,y+m_iCellSize,0,1);
