...
CCAsyncHttp::CCAsyncHttp() 
{
	...
	m_ConnectContext = CCAsyncHttpContext(CCAsyncHttpContext::CCAHC_TYPE_CONNECT, this);
	m_RequestContext = CCAsyncHttpContext(CCAsyncHttpContext::CCAHC_TYPE_REQUEST, this);
...
bool CCAsyncHttp::Get(const char* pszURL)
{
	char szLog[1024]="";
	...
	m_hInstance = InternetOpen("CCAsyncHttp", 
	...
	sprintf(szLog, "CCAsyncHttp> Download Begin : %s \n", szFileName);
#ifdef _DEBUG
	...
	{
#ifdef _DEBUG
		OutputDebugString("CCAsyncHttp::Get> CreateFile FAILED \n");
#endif
		return false;
	}
...
//		OutputDebugString(pReadBuff);
	...
	sprintf(szLog, "CCAsyncHttp> Download End : %s \n", szFileName);
#ifdef _DEBUG
	OutputDebugString(szLog);
#endif
...
void CALLBACK CCAsyncHttp::StatusCallback(HINTERNET hInternet,
								DWORD_PTR dwContext, DWORD dwInternetStatus,
								LPVOID pStatusInfo, DWORD dwStatusInfoLen)
{
	CCAsyncHttpContext* pMAHContext = (CCAsyncHttpContext*)dwContext;
	CCAsyncHttp* pAsyncHttp = pMAHContext->GetAsyncHttp();
	if (pAsyncHttp == NULL) {
	...
	case CCAsyncHttpContext::CCAHC_TYPE_CONNECT: // Connection handle
		if (dwInternetStatus == INTERNET_STATUS_HANDLE_CREATED)
		{
			...
			{
#ifdef _DEBUG
				OutputDebugString("CCAsyncHttp::StatusCallback> Connect handle created \n");
#endif
			}
			SetEvent(pAsyncHttp->m_hConnectedEvent);
		}
		break;
	case CCAsyncHttpContext::CCAHC_TYPE_REQUEST: // Request handle
		{
			...
			case INTERNET_STATUS_HANDLE_CREATED:
				{
					...
					{
#ifdef _DEBUG
						OutputDebugString("CCAsyncHttp::StatusCallback> Request handle created \n");
#endif
					}
			...
			case INTERNET_STATUS_REQUEST_SENT:
				{
					...
					{
#ifdef _DEBUG
						OutputDebugString("CCAsyncHttp::StatusCallback> Request Sent \n");
#endif
					}
				}
				break;
...
#ifdef _DEBUG
						char szLog[256];
						sprintf(szLog, "CCAsyncHttp::StatusCallback> RequestComplete(Result=%u, Error=%u) \n", 
			...
			case INTERNET_STATUS_RECEIVING_RESPONSE:
				...
				{
#ifdef _DEBUG
					OutputDebugString("CCAsyncHttp::StatusCallback> Receiving Response \n");
#endif
				}
				break;
			case INTERNET_STATUS_RESPONSE_RECEIVED:
				{
					...
					{
#ifdef _DEBUG
						OutputDebugString("CCAsyncHttp::StatusCallback> ResponseReceived \n");
#endif
					}
				}
				break;
