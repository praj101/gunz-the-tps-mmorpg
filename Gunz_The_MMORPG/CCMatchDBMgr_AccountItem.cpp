...
bool CCMatchDBMgr::GetAccountItemInfo(const int nAID, CCAccountItemNode* pOut, int* poutNodeCount, int nMaxNodeCount,
									 CCAccountItemNode* pOutExpiredItemList, int* poutExpiredItemCount, int nMaxExpiredItemCount)
{
	...
	try {
		...
		for( ; ! rs.IsEOF(); rs.MoveNext() ) {
			...
			{							
				if( bIsRentItem && pItemDesc->IsSpendableItem() ) {
					cclog( "Renewal - CCMatchDBMgr::GetAccountItemInfo - AID(%u), AIID(%u), ItemID(%u), ItemCount(%d), RentHourPeriod(%d)\n", 
		...
		*poutExpiredItemCount = nExpiredItemCount;
		...
		bException = true;
		Log("CCMatchDBMgr::GetAccountItemInfo - %s\n", e->m_strError);
		return false;
	}
...
bool CCMatchDBMgr::DeleteExpiredAccountItem(const int nAID, const int nAIID)
{
	_STATUS_DB_START;
	if (!CheckOpen()) return false;
	...
	if (nRet < 0) {
		Log("CCMatchDBMgr::DeleteExpiredAccountItem - FAILED(%d)\n", nRet);
		Log("CCMatchDBMgr::DeleteExpiredAccountItem - %s\n", strSQL.GetBuffer());
		return false;
	}
...
bool CCMatchDBMgr::BringAccountItem(const int nAID, const int nCID, const int nAIID, unsigned long int* poutCIID, 
...
bool CCMatchDBMgr::BringAccountItemStackable(const int nAID, const int nCID, const int nAIID, const int nCIID, const int nCount, unsigned long int* poutCIID, 
...
bool CCMatchDBMgr::BringBackAccountItem(const int nAID, const int nCID, const int nCIID)
{
	_STATUS_DB_START;
	if (!CheckOpen()) return false;
	...
	if( nRet != 0 ) {
		Log("CCMatchDBMgr::BringBackAccountItem - FAILED(%d), AID(%d), CID(%d), CIID(%d)\n", nRet, nAID, nCID, nCIID);
		Log("CCMatchDBMgr::BringBackAccountItem - %s\n", strSQL.GetBuffer());
		return false;
	}
...
bool CCMatchDBMgr::BringBackAccountItemStackable(const int nAID, const int nCID, const int nCIID, const int nItemCnt)
{
	_STATUS_DB_START;
	if (!CheckOpen()) return false;
	...
	if( nRet != 0 ) {
		Log("CCMatchDBMgr::BringBackAccountItemStackable - FAILED(%d), AID(%d), CID(%d), Cnt(%d)\n", nRet, nAID, nCID, nItemCnt);
		Log("CCMatchDBMgr::BringBackAccountItemStackable - %s\n", strSQL.GetBuffer());
		return false;
	}
