...
void CCMatchCheckLoopTime::SaveLoopLogFile()
{
	...
	cclog("LoopTimeGap = %5u                        [MAX : %5u]\n", dwLoopGap, m_dwMaxLoopGap);
	DWORD dwGap = 0;
	dwGap = m_dwEndWindowMsg - m_dwStartWindowMsg;
	if (dwGap > 0)
	{
		cclog("CMatchServerApp::Run() Window Message Peek [Gap : %5u]\n", dwGap);
	}
	...
	if (dwGap > 0)
	{
		cclog("Update Objects                             [Gap : %5u]\n", dwGap);
	}
	dwGap = m_dwEndStage - m_dwStartStage;
	if (dwGap > 0)
	{
		cclog("Update Stages                              [Gap : %5u]\n", dwGap);
	}
	dwGap = m_dwEndChannel - m_dwStartChannel;
	if (dwGap > 0)
	{
		cclog("m_ChannelMap.Update()                      [Gap : %5u]\n", dwGap);
	}
	dwGap = m_dwEndClan - m_dwStartClan;
	if (dwGap > 0)
	{
		cclog("m_ClanMap.Tick()                           [Gap : %5u], [%2d/ %2d %2d:%2d:%2d]\n",
	...
	if (dwGap > 0)
	{
		cclog("GetLadderMgr()->Tick()                     [Gap : %5u], [%2d/ %2d %2d:%2d:%2d]\n",
	...
	if (dwGap > 0)
	{
		cclog("Garbage Session Cleaning                   [Gap : %5u], [%2d/ %2d %2d:%2d:%2d]\n",
	...
	if (dwGap > 0)
	{
		cclog("Garbage MatchObject Cleaning               [Gap : %5u]\n", dwGap);
	}
	dwGap = m_dwEndAsyncJob - m_dwStartAsyncJob;
	if (dwGap > 0)
	{
		cclog("ProcessAsyncJob()                          [Gap : %5u]\n", dwGap);
	}
	dwGap = m_dwEndServerLog - m_dwStartServerLog;
	if (dwGap > 0)
	{
		cclog("UpdateServerLog()                          [Gap : %5u], [%2d/ %2d %2d:%2d:%2d]\n",
	...
	if (dwGap > 0)
	{
		cclog("UpdateServerStatusDB()                     [Gap : %5u], [%2d/ %2d %2d:%2d:%2d]\n", dwGap,
	...
	if (dwGap > 0)
	{
		cclog("UpdateCustomIPList()                       [Gap : %5u], [%2d/ %2d %2d:%2d:%2d]\n", dwGap,
	...
	if (dwGap > 0)
	{
        cclog("m_MatchShutdown.OnRun()                    [Gap : %5u], [%2d/ %2d %2d:%2d:%2d]\n", dwGap,
	...
	if (dwGap > 0)
	{
		cclog("GetNHNRTA().RTA()                          [Gap : %5u], [%2d/ %2d %2d:%2d:%2d]\n", dwGap,
	...
	if (dwGap > 0)
	{
		cclog("OnRun_GameGuard()                          [Gap : %5u], [%2d/ %2d %2d:%2d:%2d]\n", dwGap,
	...
	if (dwGap > 0)
	{
		cclog("OnRun_XTrap()                              [Gap : %5u]\n", dwGap);
	}
	dwGap = m_dwEndMonitor - m_dwStartMonitor;
	if (dwGap > 0)
	{
		cclog("m_Monitor.Run()                            [Gap : %5u]\n", dwGap);
	}
	dwGap = m_dwEndKillTracker - m_dwStartKillTracker;
	if (dwGap > 0)
	{
		cclog("GetKillTracker().Update()                  [Gap : %5u], [%2d/ %2d %2d:%2d:%2d]\n",dwGap,
	...
	cclog("LOOP LOG END ======= [Loop End Time   : %2d/ %2d %2d:%2d:%2d] ===================\n", st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond);
