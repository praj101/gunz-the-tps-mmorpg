...
CCDrawContextR2::CCDrawContextR2(LPDIRECT3DDEVICE9 pd3dDevice)
{
...
CCDrawContextR2::~CCDrawContextR2()
{
}
...
void CCDrawContextR2::SetPixel(int x, int y, sColor& color)
{
...
void CCDrawContextR2::HLine(int x, int y, int len)
{
	Line(x, y, x+len, y);
}

void CCDrawContextR2::VLine(int x, int y, int len)
{
	Line(x, y, x, y+len);
}

void CCDrawContextR2::Line(int sx, int sy, int ex, int ey)
{
	m_pd3dDevice->SetFVF(D3DFVF_PIXEL2DVERTEX);
	m_pd3dDevice->SetPixelShader(NULL);
...
void CCDrawContextR2::Rectangle(int x, int y, int cx, int cy)
{
	x += m_Origin.x;
	y += m_Origin.y;
...
void CCDrawContextR2::FillRectangleW(int x, int y, int cx, int cy)
{
...
void CCDrawContextR2::FillRectangle(int x, int y, int cx, int cy)
{
	m_pd3dDevice->SetFVF(D3DFVF_PIXEL2DVERTEX);
	m_pd3dDevice->SetPixelShader(NULL);
...
void CCDrawContextR2::Draw(CCBitmap *pBitmap, int x, int y, int w, int h, int sx, int sy, int sw, int sh, bool bMirrorX, bool bMirrorY )
{
...
void CCDrawContextR2::DrawEx(int tx1, int ty1, int tx2, int ty2, int tx3, int ty3, int tx4, int ty4)
{
...
bool CCDrawContextR2::BeginFont()
{
	CCFontR2* pFont = (CCFontR2*)m_pFont;
...
int CCDrawContextR2::Text(int x, int y, const char* szText)
{
	CCFontR2* pFont = (CCFontR2*)m_pFont;
...
void CCDrawContextR2::SetClipRect(sRect& r)
{
	CCDrawContext::SetClipRect(r);
...
bool CCBitmapR2::Create(const char* szAliasName, LPDIRECT3DDEVICE9 pd3dDevice, const char* szFileName,bool bUseFileSystem,DWORD colorkey)
{
	CCBitmap::Create(szAliasName);
...
/*
bool CCBitmapR2::Create(const char* szAliasName, LPDIRECT3DDEVICE9 pd3dDevice, void* pData, int nSize)
{
	CCBitmap::Create(szAliasName);
...
void CCBitmapR2::UnloadTextureTemporarily()
{
...
int CCBitmapR2::GetWidth()
{
	return m_nWidth;
}

int CCBitmapR2::GetHeight()
{
	return m_nHeight;
}
...
DWORD CCBitmapR2::m_dwStateBlock;

// 현재는 state block을 사용하지 않는다.
//#define USE_STATEBLOCK

void CCBitmapR2::BeginState(CCDrawEffect effect)
{
...
void CCBitmapR2::CheckDrawMode(float* fuv)
{
	if(m_DrawMode) {
...
void CCBitmapR2::Draw(float x, float y, float w, float h, float sx, float sy, float sw, float sh, 
					 DWORD dwColor, CCDrawEffect effect, bool bMirrorX, bool bMirrorY)
{
	/*
...
void CCBitmapR2::DrawEx(float tx1, float ty1, float tx2, float ty2, 
			float tx3, float ty3, float tx4, float ty4, DWORD dwColor, CCDrawEffect effect)
{
	if (!m_pTexture) return;
...
void CCBitmapR2::OnLostDevice()
{
	//if(m_pSprite!=NULL) m_pSprite->OnLostDevice();
}

void CCBitmapR2::OnResetDevice()
{
...
bool CCFontR2::Create(const char* szAliasName, const char* szFontName, int nHeight, float fScale, bool bBold, bool bItalic, int nOutlineStyle, int nCacheSize, bool bAntiAlias, DWORD nColorArg1, DWORD nColorArg2)
{
...
void CCFontR2::Destroy()
{
	m_Font.Destroy();
}

int CCFontR2::GetHeight()
{
	return int(m_Font.GetHeight()*m_fScale);
}

int CCFontR2::GetWidth(const char* szText, int nSize)
{
