...
/// NPC 공격 타입 1Byte보다 커진다면 CCQuestNPCInfo.nNPCAttackTypes을 수정해 줘야 한다.
enum CCQUEST_NPC_ATTACK
{
...
struct CCQuestNPCInfo
{
	// 기본 정보
	CCQUEST_NPC			nID;				///< id
	CCQUEST_NPC_GRADE	nGrade;				///< 등급
	char				szName[256];		///< 이름
	char				szDesc[256];		///< 설명
	BYTE				nNPCAttackTypes;	///< 공격타입	- CCQUEST_NPC_ATTACK형의 SET
	float				fSpeed;				///< 속도
	float				fWeight;			///< 몸무게
	...
	void SetDefault()
	{
		...
		fCollHeight			= 180.0f;
		bColPick			= false;
		vScale				= CCVector(1.0f,1.0f,1.0f);
		vColor				= CCVector(0.6f,0.6f,0.6f);
		nSpawnWeight		= 100;
	...
	CCQuestNPCSpawnType GetSpawnType();		///< 스폰 타입 반환
};
...
struct CCQuestNPCGlobalAIValue
{
	// shaking ratio
...
/// NPC 정보 관리자 클래스
class CCQuestNPCCatalogue : public map<CCQUEST_NPC, CCQuestNPCInfo*>
{
private:
...
public :
	CCQuestNPCInfo* GetIndexInfo( int nIndex );

public:
	CCQuestNPCCatalogue();													///< 생성자
	~CCQuestNPCCatalogue();													///< 소멸자
	...
	CCQuestNPCInfo* GetInfo(CCQUEST_NPC nNpc);								///< NPC 정보 반환
	CCQuestNPCInfo* GetPageInfo( int nPage);									///< 페이지로부터 NPC 정보 반환

	CCQuestNPCGlobalAIValue* GetGlobalAIValue() { return &m_GlobalAIValue; }
};
...
struct CCQuestNPCSetInfo
{
	int					nID;				///< ID
	char				szName[16];			///< 이름
	CCQUEST_NPC			nBaseNPC;			///< 기본 베이스 NPC
	vector<CCNPCSetNPC>	vecNPCs;			///< 등장할 NPC
};

/// NPC Set 정보 관리자 클래스
class CCQuestNPCSetCatalogue : public map<int, CCQuestNPCSetInfo*>
{
private:
	map<string, CCQuestNPCSetInfo*>		m_NameMap;
	// 함수
	void Clear();
	void ParseNPCSet(CCXmlElement& element);
	void Insert(CCQuestNPCSetInfo* pNPCSetInfo);
public:
	CCQuestNPCSetCatalogue();											///< 생성자
	~CCQuestNPCSetCatalogue();											///< 소멸자

	bool ReadXml(const char* szFileName);								///< xml로부터 npc정보를 읽는다.
	bool ReadXml(CCZFileSystem* pFileSystem,const char* szFileName);		///< xml로부터 npc정보를 읽는다.

	CCQuestNPCSetInfo* GetInfo(int nID);									///< NPC Set 정보 반환
	CCQuestNPCSetInfo* GetInfo(const char* szName);						///< NPC Set 정보 반환
};