	...
	if(pNode->nPolygon)
	{
		...
		if(nCount)
		{
			...
			HRESULT hr=RGetDevice()->DrawIndexedPrimitive(D3DPT_TRIANGLELIST,
				pNode->m_iBaseVertexIndex,0,pNode->m_iVertices,
...
*/
	...
	int nCount=m_iMaterial*max(1,m_iLightmap);

	for(i=0;i<nCount;i++)
	{
		int nMaterial = i % m_iMaterial;
		int nLightmap = i / m_iMaterial;
...
void RBspObject::SetObjectLight(rvector vPos)
{
	...
	for(RLightList::iterator i=pllist->begin();i!=pllist->end();i++) {
		...
		fdistance = Magnitude(plight->sPosition - vPos);
...
bool RBspObject::ReadString(CCZFile *pfile,char *buffer,int nBufferSize)
{
	int nCount=0;
	do{
		pfile->Read(buffer,1);
...
bool RBspObject::UpdateIndexBuffer()
{
	...
	memcpy(pIndices,m_pOcIndices,sizeof(WORD)*m_iIndices);
...
bool RBspObject::Open_MaterialList(CCXmlElement *pElement)
{
	...
	m_iMaterial=ml.size();
	m_iMaterial=m_iMaterial+1;

	m_pMaterials=new RBSPMATERIAL[m_iMaterial];
	...
	for(RLightList::iterator i=llist.begin();i!=llist.end();i++)
	{
		...
		if(strnicmp(plight->Name.c_str(),RTOK_MAX_OBJLIGHT,strlen(RTOK_MAX_OBJLIGHT))==0)
			...
			if( strnicmp(plight->Name.c_str(), "sun_omni", 8 ) == 0 )
			{
				...
				psunlight->sPosition=plight->sPosition;
...
//		RLightList *pllist=GetMapLightList();
		...
		for( RLightList::iterator rlit=pllist->begin();rlit != pllist->end(); rlit++ ) {
			...
			if(plight) {
				...
				float fdistance=Magnitude(plight->sPosition-center);
...
bool RBspObject::OpenRs(const char *filename)
{
	...
	file.Read(&m_iBspNodeCount,sizeof(int));
	file.Read(&m_iBspPolygon,sizeof(int));
	file.Read(&m_iBspVertices,sizeof(int));
	file.Read(&m_iBspIndices,sizeof(int));

	file.Read(&m_iNodeCount,sizeof(int));
	file.Read(&m_iPolygon,sizeof(int));
	file.Read(&m_iVertices,sizeof(int));
	file.Read(&m_iIndices,sizeof(int));

	m_pOcRoot = new RSBspNode[m_iNodeCount];
	m_pOcInfo = new RPOLYGONINFO[m_iPolygon];
	m_pOcVertices = new BSPVERTEX[m_iVertices];
	m_pOcIndices = new WORD[m_iIndices];
...
bool RBspObject::OpenBsp(const char *filename)
{
	...
	if(m_iBspNodeCount!=nBspNodeCount || m_iBspPolygon!=nBspPolygon ||
		m_iBspVertices!=nBspVertices || m_iBspIndices!=nBspIndices ) 
	{
		file.Close();
		return false;
	}

	m_pBspRoot=new RSBspNode[m_iBspNodeCount];
	m_pBspInfo=new RPOLYGONINFO[m_iBspPolygon];
	m_pBspVertices=new BSPVERTEX[m_iBspVertices];
//	m_pBspIndices = new WORD[m_iBspIndices];
...
//	g_pLPIndices=m_pBspIndices;
	...
	_ASSERT(m_pBspRoot+m_iBspNodeCount>g_pLPNode);
	
	file.Close();
	return true;
}

bool RBspObject::Open_ColNodes(RSolidBspNode *pNode,CCZFile *pfile)
{
	...
	pfile->Seek(nPolygon*4*sizeof(rvector),CCZFile::current);
...
//	if(m_ppLightmapTextures) return true;	// 이미 로드되어있다
	...
	int i;
//	int *pLightmapInfo=new int[m_iConvexPolygon];		// lightmap 에 중복이 제거된 인덱스.
...
//		delete memory;
	...
	file.Read(pOrder,sizeof(int)*m_iPolygon);

	// 저장될때의 폴리곤 순서를 참고하여 폴리곤당 라이트맵 번호를 갱신한다
	for(i=0;i<m_iPolygon;i++)
...
bool RBspObject::Open_ConvexPolygons(CCZFile *pfile)
{
	int nConvexVertices;//,nSourcePolygon;

	pfile->Read(&m_iConvexPolygon,sizeof(int));
	pfile->Read(&nConvexVertices,sizeof(int));

	// 이 정보들은 라이트맵을 생성할때 필요하므로 런타임에서는 읽을필요가 없다

	if(m_OpenMode==ROF_RUNTIME) {
		for(int i=0;i<m_iConvexPolygon;i++)
		{
			pfile->Seek(sizeof(int)+sizeof(DWORD)+sizeof(rplane)+sizeof(float),CCZFile::current);
			
			int nVertices;
			pfile->Read(&nVertices,sizeof(int));

			pfile->Seek(nVertices*2*sizeof(rvector),CCZFile::current);
		}
		return true;
	}

	m_pConvexPolygons=new RCONVEXPOLYGONINFO[m_iConvexPolygon];
...
//				WORD base = pInfo->pVertices - m_pOcVertices;
				WORD base = pInfo->pVertices - ( m_pOcVertices + pNode->m_iBaseVertexIndex );
		...
		int lastmat=pNode->ppInfoSorted[0]->nMaterial+pNode->ppInfoSorted[0]->nLightmapTexture*m_iMaterial;
		...
		for(int j=1;j<pNode->nPolygon+1;j++)
		{
			int nMatIndex = (j==pNode->nPolygon) ? -999 :	// 마지막이면 정리하자
				pNode->ppInfoSorted[j]->nMaterial+pNode->ppInfoSorted[j]->nLightmapTexture*m_iMaterial;
	...
	if(pNode->nPolygon)
	{
		...
		for(int j=0;j<pNode->nPolygon-1;j++)
		{
			for(int k=j+1;k<pNode->nPolygon;k++)
			{
				...
				if(pj->nLightmapTexture*m_iMaterial + pj->nMaterial > pk->nLightmapTexture*m_iMaterial + pk->nMaterial )
				{
...
bool RBspObject::Open_Nodes(RSBspNode *pNode,CCZFile *pfile)
{
	...
	if(pNode->nPolygon)
	{
		...
		pNode->m_iBaseVertexIndex = g_pLPVertices - m_pOcVertices;
...
//			Normalize(nor);
			...
			else
			{
				...
				if(nMaterial<0 || nMaterial>=m_iMaterial) nMaterial=0;	

				pInfo->nMaterial=nMaterial;
				pInfo->dwFlags|=m_pMaterials[nMaterial].dwFlags;
			}
			_ASSERT(pInfo->nMaterial<m_iMaterial);
		...
		pNode->m_iVertices = (g_pLPVertices - m_pOcVertices) - pNode->m_iBaseVertexIndex;
	}
...
{
	if(/*m_iPolygon*3 > 65530 || */m_iPolygon==0 ) return false;

	g_nCreatingPosition=0;

	SAFE_RELEASE(m_pVertexBuffer);

	HRESULT hr=RGetDevice()->CreateVertexBuffer( sizeof(BSPVERTEX)*m_iVertices, D3DUSAGE_WRITEONLY , BSP_FVF, D3DPOOL_MANAGED, &m_pVertexBuffer ,NULL);
	_ASSERT(hr==D3D_OK);
...
bool RBspObject::UpdateVertexBuffer()
{
	...
	_ASSERT(hr==D3D_OK);
	memcpy(pVer,m_pOcVertices,sizeof(BSPVERTEX)*m_iVertices);
...
bool RBspObject::GeneratePathData(const char *filename,float fAngle,float fToler)
{
	...
	for(i=0;i<m_iConvexPolygon;i++)
	{
		if(sin(fAngle)<fabs(m_pConvexPolygons[i].plane.c) && (m_pConvexPolygons[i].dwFlags & RM_FLAG_NOTWALKABLE)==0 )	// normal 의 z 값
		{
			...
			ppathnode->m_iSourceID=i;
			ppathnode->m_iGroupID=-1;
	...
	GeneratePathNodeTable();
	return m_PathNodes.Save(filename,m_iConvexPolygon);
}
...
void RBspObject::GeneratePathNodeTable()
{
...

	{
		...
		{
			m_pConvexPolygons[m_PathNodes[i]->m_iSourceID].nPathNode=i;
		}
	}
}
...
bool RBspObject::GenerateLightmap(const char *filename,int nMaxlightmapsize,int nMinLightmapSize,int nSuperSample,float fToler,RGENERATELIGHTMAPCALLBACK pProgressFn)
{
	...
	int	*pSourceLightmap=new int[m_iConvexPolygon];
	...
	for(i=0;i<m_iConvexPolygon;i++)
	{
		...
		if(pProgressFn)
		{
			bool bContinue=pProgressFn((float)i/(float)m_iConvexPolygon);
			if(!bContinue)
			{
				bReturnValue=false;
				goto clearandexit;
		...
		*/
		{
			...
			for(RLightList::iterator light=m_StaticMapLightList.begin();light!=m_StaticMapLightList.end();light++){
				...
				// 유효거리가 벗어나면 영향이 없다
				if(GetDistance((*light)->sPosition,poly->plane)>(*light)->fAttnEnd) continue;

				for(int iv=0;iv<poly->nVertices;iv++)
				{
					// 하나라도 빛을 받는 normal 이 있다면
					if(DotProduct((*light)->sPosition-poly->pVertices[iv],poly->pNormals[iv])>0) {
			...
			for(l=0;l<nLight;l++)
			{
				...
				for(j=0;j<lightmapsize+1;j++)			// v 
				{
					for(k=0;k<lightmapsize+1;k++)		// u
					{
						...
						float fDistanceToPolygon=Magnitude(position-plight->sPosition);

						RBSPPICKINFO bpi;
						if(PickShadow(plight->sPosition,position,&bpi)) 
						{
							float fDistanceToPickPos=Magnitude(bpi.PickPos-plight->sPosition);
...
//								rvector origin=plight->sPosition*inv;
//								rvector target=position*inv;

								rvector origin;
								D3DXVec3TransformCoord(&origin,&plight->sPosition,&inv);
								rvector target;
								D3DXVec3TransformCoord(&target,&position,&inv);

								rvector dir=target-origin;
								rvector dirorigin=position-plight->sPosition;
								...
								if( // 바운딩박스테스트후
									bBBTest &&
									// pick 되어지면..
									poi->pVisualMesh->Pick(plight->sPosition,dirorigin,&vOut,&t))
								{
									rvector PickPos=plight->sPosition+vOut*t;
									// 현재 위치를 리턴하는 값이 좀 이상해서 주석처리해둠.
									//float fDistanceToPickPos=Magnitude(PickPos-plight->sPosition);
									//if(fDistanceToPolygon>fDistanceToPickPos+fToler)
										isshadow[k*(lightmapsize+1)+j]=true;
								}
							}
...
//							if(1)
							{
								...
								for(m=0;m<nSuperSample;m++)
								{
									for(n=0;n<nSuperSample;n++)
									{
										...
										if(!bShadow)
										{
											rvector dpos=plight->sPosition-position;
	...
	fwrite(&m_iConvexPolygon,sizeof(int),1,file);
	fwrite(&m_iNodeCount,sizeof(int),1,file);

	// 적당한 크기에 합쳐져 넣어진 라이트맵을 저장한다.
	m_iLightmap=m_LightmapList.size();
	fwrite(&m_iLightmap,sizeof(int),1,file);
...
//		/*
	...
	*/
	...
	/*
	for(int i=0;i<m_iBspPolygon*3;i++)
...
//	pd3dDevice->SetRenderState(D3DRS_CULLMODE, D3DCULL_CW );
	...
	for(int i=0;i<m_iMaterial;i++)
	{
		...
		*/
		...
		if((m_pMaterials[i % m_iMaterial].dwFlags & RM_FLAG_TWOSIDED) == 0 )
			RGetDevice()->SetRenderState(D3DRS_CULLMODE, D3DCULL_CW );
		else
			RGetDevice()->SetRenderState(D3DRS_CULLMODE, D3DCULL_NONE );

		if((m_pMaterials[i % m_iMaterial].dwFlags & RM_FLAG_ADDITIVE ) == 0 )
		{
			int nMaterial = i % m_iMaterial;
			RBaseTexture *pTex=m_pMaterials[nMaterial].texture;
			if(pTex)
