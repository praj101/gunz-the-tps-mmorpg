...
/// 퀘스트의 게임 상태
enum CCQuestCombatState
{
	CCQUEST_COMBAT_NONE		= 0,
	CCQUEST_COMBAT_PREPARE	= 1,	// 대기중
	CCQUEST_COMBAT_PLAY		= 2,	// 전투 시작
	CCQUEST_COMBAT_COMPLETED	= 3		// 전투종료 - 다음맵으로 이동
};



enum CCQuestItemType
{
	CCQIT_PAGE = 1,
	CCQIT_SKULL,
	CCQIT_FRESH,
...
#define CCSQITRES_NOR  1	// 일반시나리오에 대한 희생 아이템만 있는 상황.
#define CCSQITRES_SPC  2	// 특별시나리오에 해당하는 희생아이템이 있음.
#define CCSQITRES_INV  3	// 해당 QL에대한 희생아이템 정보 테이블이 없음. 이경우는 맞지 않는 희생 아이템이 올려져 있을경우.
#define CCSQITRES_DUP  4 // 양쪽 슬롯에 같은 아이템이 올려져 있음.
#define CCSQITRES_EMP  5 // 양쪽 슬롯이 모두 비어 있음. 이 상태는 QL값을1로 해줘야 함.
#define CCSQITRES_ERR -1	// 에러... 테이블에서 해당 QL을 찾을수 없음. QL = 0 or QL값이 현재 구성된 MAX QL보다 클경우.


// 몬스터 도감 관련. 
#define CCSMB_UNKNOW		0	// 해당 몬스터에 대한 정보는 아는게 없음.
#define CCSMB_KNOW			1	// 해당 몬스터에 대한 정보는 이미 알고 있음.
#define CCSMB_NEWINFO		2	// 새롭게 획득. 이 정보는 도감에 등록을 시키고 MSMB_KNOW로 바꿔줘야 함.
