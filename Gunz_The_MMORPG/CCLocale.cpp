...
int	CCLocale::GetCharsetFromLang(LANGID langid)
{
...
int	CCLocale::GetCodePageFromCharset(int nCharset)
{
	switch (nCharset)
	{
		case HANGUL_CHARSET:
...
int	CCLocale::GetCodePageFromLang(LANGID langid)
{
	return GetCodePageFromCharset(GetCharsetFromLang(langid));
}


std::string CCLocale::TransCode(const wchar_t *pwszString, int nCodePage)
{
	std::string strRet; 
...
std::string CCLocale::TransCode(const char *pszString, int nOldCodePage, int nNewCodePage)
{
	std::string strRet; 
...
std::string CCLocale::ConvUTF8ToAnsi(const char* pszString, LANGID langid)
{
...
std::string CCLocale::ConvAnsiToUTF8(const char* pszString, LANGID langid)
{
	if (pszString == NULL) return std::string("");
	int nCodePage = GetCodePageFromLang(langid);
